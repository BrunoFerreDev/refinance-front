...
export default {
  ...
  async getCanchas(page = 0, size = 100) {
    try {
      ...
      if (!canchas) {
        canchas = [
          ...
          { idCancha: 2, nombre: "Cancha Auxiliar B" },
          {
            idCancha: 3,
  ...
  async getAranceles(page = 0, size = 10) {
    try {
  ...
  async createArancel(payload) {
    try {
      ...
      const canchas = await this.getCanchas();
      const canchaObj =
  ...
  async updateArancel(idArancel, payload) {
    try {
  ...
  async toggleArancelActivo(idArancel, activo) {
    try {
      const response = await apiClient.put(
  ...
  async updateCancha(idCancha, payload) {
    try {
      const response = await apiClient.put(`/canchas/${idCancha}`, payload);
      return response.data;
    } catch (error) {
      console.warn("Error al actualizar cancha, usando simulación local:", error.message);
      const localKey = "reffinance_canchas";
      const stored = localStorage.getItem(localKey);
      if (stored) {
