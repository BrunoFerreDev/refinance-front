<template>
  <header class="h-20 bg-white border-b border-reffinance-border flex items-center justify-between px-4 sm:px-8 shrink-0 select-none">
    ...
    <div class="flex items-center space-x-3 sm:space-x-6 shrink-0">
      ...
      <div v-if="isDemo" class="hidden lg:flex items-center px-3 py-1 bg-amber-50 border border-amber-200 text-amber-700 rounded-full text-xs font-semibold select-none animate-pulse">
        <span class="w-1.5 h-1.5 bg-amber-500 rounded-full mr-2"></span>
        Modo Simulación
      </div>

      <!-- Action Buttons -->
      <div class="flex items-center space-x-2 text-slate-400 shrink-0">
        <button @click="showNotif" class="p-2 hover:bg-slate-50 hover:text-slate-700 rounded-full transition-colors relative">
          <Bell class="w-5 h-5" />
          <span class="absolute top-1 right-1 w-2.5 h-2.5 bg-rose-500 border-2 border-white rounded-full"></span>
        </button>
        <button @click="showHelp" class="p-2 hover:bg-slate-50 hover:text-slate-700 rounded-full transition-colors">
      ...
      <div class="flex items-center space-x-3 shrink-0">
        <div class="text-right hidden sm:block shrink-0">
          <p class="text-sm font-bold text-reffinance-navy font-outfit leading-tight">Admin Usuario</p>
          <p class="text-[10px] text-slate-400 font-semibold leading-tight">Super Administrador</p>
        </div>
        <!-- User Avatar -->
        <div class="w-10 h-10 rounded-lg overflow-hidden bg-reffinance-navy text-white flex items-center justify-center font-bold text-sm shadow-sm shrink-0">
          AD
        </div>
      </div>
    </div>
